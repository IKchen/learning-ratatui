...
use crate::{app::App, event::{EventHandler,Event},error::MyError};
pub struct Tui{
    pub terminal:Terminal<CrosstermBackend<std::io::Stdout>>,//终端
...
impl Tui{
    pub fn new()->Result<Self,MyError>{
       ...
       // let task =tokio::spawn(async{});//new时，就初始化task，避免new之后的实例，使用时单独初始化
        ...
        Ok(Self{terminal,handler,tick_rate,frame_rate,cancellation_token} )
    }
    ...
    pub fn start(& mut self){
      ...
      let _cancellation_token = self.cancellation_token.clone();
       self.handler.run(tick_delay,render_delay,_cancellation_token);
    }
    //停止程序
    pub fn stop(& mut self)->Result<(),MyError>{
      self.cancel();
      let mut counter = 0;
      while !self.handler.task.is_finished() {
        std::thread::sleep(Duration::from_millis(1));
        counter += 1;
        //等待 50 ms，50ms 之后没有完成task ，则强制终止任务 
        if counter > 50 {
          self.handler.task.abort();
        }
    ...
    //进入tui 模式
    pub fn enter(&mut self)->Result<(),MyError>{
...
/*     fn reset() -> Result<(),std::io::Error> {
 ...
 /*      pub fn draw(&mut self, app: &mut App) -> Result<(),std::io::Error> {
      ...
      //挂起应用
      pub fn suspend(& mut self)->Result<(),MyError>{
